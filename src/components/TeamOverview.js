...
const TeamOverview = () => {
  ...
  useEffect(() => {
    ...
    teamOverviewData.forEach((employee) => {
      ...
      criteriaData.forEach((category) => {
        mockSkills[employee.id][category.category] =
  ...
  const getBadgeColor = (value) => {
    ...
    if (valueStr.includes("high") || valueStr.includes("excellent"))
      return "success";
    if (valueStr.includes("medium") || valueStr.includes("good"))
      return "primary";
    if (valueStr.includes("low") || valueStr.includes("poor")) return "warning";
    if (valueStr.includes("risk") || valueStr.includes("critical"))
      return "danger";
                ...
                {filteredEmployees.map((employee, index) => (
                  ...
                  >
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      {employee.risk && (
                        <span
                          className={`badge bg-${getBadgeColor(employee.risk)}`}
                        >
                          {employee.risk}
                        </span>
                      )}
                ...
                <Row className="mb-4">
                  <Col md={6}>
                    ...
                    <p>
                      ...
                      {selectedEmployee.skill_expertise || "Not specified"}
                    </p>
                    <p>
                      <strong>Value:</strong>{" "}
                      {selectedEmployee.value || "Not specified"}
                    </p>
                    <p>
                      <strong>Potential:</strong>{" "}
                      {selectedEmployee.potential || "Not specified"}
                    </p>
                  </Col>
                  <Col md={6}>
                    ...
                    <p>
                      ...
                      {selectedEmployee.risk || "Not specified"}
                    </p>
                    <p>
                      <strong>Salary Plan:</strong>{" "}
                      {selectedEmployee.salary_increase_plan || "Not specified"}
                    </p>
                    <p>
                      <strong>Comments:</strong>{" "}
                      {selectedEmployee.free_comment || "None"}
                    </p>
                  </Col>
                </Row>
                      ...
                      {criteria.map((category, index) => (
                        <tr key={index}>
                          ...
                          {[1, 2, 3, 4, 5].map((level) => (
                            <td
                              key={level}
                              className={`text-center ${getLevelClass(level)} ${
                                employeeSkills[selectedEmployee.id]?.[
                                  category.category
                                ] === level
                                  ? "border border-dark"
                                  : ""
                              ...
                              onClick={() =>
                                handleSkillLevelChange(category.category, level)
                              }
                              style={{
                  ...
                  <ul>
                    {Object.entries(employeeSkills[selectedEmployee.id] || {})
                      ...
                      .map(([category, level], index) => (
                        <li key={index}>
                          Focus on improving <strong>{category}</strong> from
